...
import { exchangeFacebookCode } from "@/lib/socialAuth";

export const dynamic = "force-dynamic";

// POST { code } → { accessToken, refreshToken, member, isNew }
// 🔴 แอปส่งมาแค่ code — การแลก token ใช้ app secret ซึ่งอยู่ฝั่งเซิร์ฟเวอร์เท่านั้น
export async function POST(req: NextRequest) {
  try {
    const { code } = (await req.json()) as { code?: string };
    if (!code) return NextResponse.json({ error: "code required" }, { status: 400 });

    const id = await exchangeFacebookCode(code);
    ...
    return NextResponse.json({ error: "facebook sign-in failed" }, { status: 401 });
  }
}
