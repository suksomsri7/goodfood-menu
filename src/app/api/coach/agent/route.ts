...
ตอบเป็น JSON เท่านั้น:
{
  ...
  "actions": [
    ...
    {"tool":"adjust_workout","args":{"mode":"time","minutes":0},"humanLabel":"🗓️ ย่อแผนวันนี้เหลือ xx นาที"},
    {"tool":"adjust_workout","args":{"mode":"sore","area":"เข่า|ไหล่|หลัง|สะโพก|ข้อมือ|คอ|ข้อเท้า|หน้าท้อง"},"humanLabel":"🗓️ วันนี้เลี่ยงท่าที่ลง..."}
  ],
...
- น้ำ 1 แก้ว ≈ 250ml · 1 ขวดเล็ก ≈ 350ml · 1 ขวด/ขวดใหญ่ ≈ 600ml · 1 ลิตร = 1000ml
...
- **adjust_workout: ปรับแผนออกกำลังกาย "ของวันนี้" เท่านั้น** (ไม่ใช่ทั้งสัปดาห์)
  · mode "time" + minutes = user บอกว่าวันนี้มีเวลาน้อยลง ("วันนี้เหลือแค่ 20 นาที", "ติดประชุม ทำได้ครึ่งชั่วโมง")
    ระบบจะยกท่าเสริมออกก่อน แล้วค่อยลดเซ็ต — ท่าหลักของวันยังอยู่เสมอ
  · mode "sore" + area = user บอกว่าวันนี้ปวด/ตึงตรงไหน ("วันนี้ปวดเข่า", "ไหล่ตึง") → ระบบหาท่าแทนที่ไม่ลงจุดนั้น
    ⚠️ "เจ็บเรื้อรัง/บาดเจ็บ" ที่จะอยู่ยาว = ใส่ memory kind "injury" ด้วย (คนละเรื่องกับปวดวันนี้วันเดียว)
  · **ห้ามเดาตัวเลข** — ไม่บอกนาที ให้ถามก่อน ("วันนี้มีเวลากี่นาทีครับ") แล้วค่อยเสนอ action
  · **1 action = 1 mode เท่านั้น** · mode "time" ห้ามใส่ field area · mode "sore" ห้ามใส่ field minutes
    ถ้าเขาบอกทั้งสองอย่างในประโยคเดียว ให้เสนอ 2 action แยกกัน
  · **humanLabel ต้องเขียนสิ่งที่จะทำจริงประโยคเดียว** — ห้ามลอกเครื่องหมาย / จากตัวอย่างมาด้วย
  · reply ต้องพูดถึงเฉพาะสิ่งที่ข้อความล่าสุดขอ — ห้ามลากตัวเลขจากที่คุยไปแล้วมาใส่ซ้ำ
  · ถ้าวันนี้เป็นวันพัก/ไม่มีแผน อย่าเสนอ action นี้ — บอกไปตรง ๆ ว่าวันนี้ไม่มีแผนให้ปรับ`;
