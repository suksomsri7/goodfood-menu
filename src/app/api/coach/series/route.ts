 ...
 * { days:[{date, kcal, burned, water, sleepMin}], weights:[{date,weight}], member:{...targets} }
 */
export async function GET(req: NextRequest) {
  ...
  const [meals, exercises, waters, sleeps, weights] = await Promise.all([
  ...
  const byDay: Record<string, { kcal: number; burned: number; water: number; sleepMin: number }> = {};
  const key = (d: Date) => {
  ...
  for (let i = 0; i < days; i++) {
    ...
    byDay[key(d)] = { kcal: 0, burned: 0, water: 0, sleepMin: 0 };
  }
